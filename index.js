...
app.all('/upload', (request, response) => {
    if (request.method == 'GET') {
        ...
        let form = new formidable.IncomingForm({ multiples: false, allowEmptyFiles: true, minFileSize: 0 }) // ไม่ต้องการหลายไฟล์, อนุญาตให้ไฟล์ว่าง 0 bytes โดยไม่มี Error, ไฟล์ขนาด 0 bytes ผ่านได้
        form.parse(request, (err, fields, files) => {
            ...
            if (!fields.overwrite && fileSystem.existsSync(newfile)) {
                let oldName = newName.split('.') // แยกชื่อไฟล์ด้วย . กรณีซ้ำกัน
            ...
            console.log('upfile.originalFilename => ' + newName)
            try {
            ...
            if (upfile.mimetype && upfile.mimetype.match('image/')) {
                data = { 
                    file: 'upload/' + newName,
                    fileInfo: {
                        name: newName,
